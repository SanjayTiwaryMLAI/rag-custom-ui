...
export const PromptTemplate: FC<Props> = ({
  onChangePrompt,
  showPromptWindow,
  handleShowPromptWindow,
}) => {
  const {
    state: {
  ...
  const isPromptTemplateValid = () => {
    let errors = "";
    const requiredPlaceholders = RAGEnabled
      ? ["history", "input", "context"]
      : ["history", "input"];
    if (
      value.length < MIN_PROMPT_TEMPLATE_LENGTH ||
      value.length > MAX_PROMPT_TEMPLATE_LENGTH
    ) {
      errors +=
